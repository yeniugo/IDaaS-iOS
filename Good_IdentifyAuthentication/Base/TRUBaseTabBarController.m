...
#define ARRAY_SIZE(a) sizeof(a)/sizeof(a[0]) // 一些常见的越狱后的特征 const char*
...
- (void)viewDidLoad {
    ...
    [HAMLogOutputWindow printLog:uuid];
...
//        [HAMLogOutputWindow printLog:@"TRUBaseTabBarController viewDidLoad2"];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (delegate.thirdAwakeTokenStatus) {
                if ((delegate.thirdAwakeTokenStatus) && ([TRUFingerGesUtil getLoginAuthFingerType]!=TRULoginAuthFingerTypeNone||[TRUFingerGesUtil getLoginAuthGesType]!=TRULoginAuthGesTypeNone)) {
//                    [TRUEnterAPPAuthView showAuthViewWithCompletionBlock:^{
//                        [weakSelf getNetToken];
//                    }];
//                    [HAMLogOutputWindow printLog:@"base 2"];
                    if (delegate.thirdAwakeTokenStatus) {
                        if (delegate.appPushVC!=nil) {
//                            [HAMLogOutputWindow printLog:@"base 3"];
                            if ([delegate.window.rootViewController isKindOfClass:[UINavigationController class]]) {
//                                [HAMLogOutputWindow printLog:@"base 4"];
...
//                                    }];
//                                    [HAMLogOutputWindow printLog:@"base 5"];
                                    [rootnav pushViewController:delegate.appPushVC animated:YES];
                                    delegate.appPushVC = nil;
                                });
                            }
...
//                    [TRUEnterAPPAuthView showAuthView];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(showPushToken) name:@"pushAuthVC" object:nil];
...
//        }
//    }
    [self showSchemeViewController];
}

- (void)showSchemeViewController{
...
//            [HAMLogOutputWindow printLog:@"TRUBaseTabBarController pushtoken2"];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [(UINavigationController *)(delegate.window.rootViewController) pushViewController:delegate.tokenPushVC animated:YES];
