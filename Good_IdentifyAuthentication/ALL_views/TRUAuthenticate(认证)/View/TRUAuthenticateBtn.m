...
//    self.layer.frame = self.frame;
    ...
    if (IS_IPAD) {
        iconImageH = 53.5/300.0*self.bounds.size.height;
        iconImageW = 58.5/266.5*self.bounds.size.width;
    }
    if ([self.textLabel.text isEqualToString:@"扫一扫"]) {
        iconImageH = 32*PointHeightRatio6;
        iconImageW = 32*PointHeightRatio6;
        if (IS_IPAD) {
            iconImageH = 50.0/300.0*self.bounds.size.height;
            iconImageW = 50.0/266.0*self.bounds.size.width;
        }
//        YCLog(@"重新布局1");
    }else{
//        YCLog(@"重新布局2");
    ...
    if (IS_IPAD) {
        iconImageY = 91.5/300*self.bounds.size.height;
    }
