...
- (void)setDataSources{
    ...
    userName.titleStr = @"身份证";
    NSString *shenfenzheng;
    if([TRUUserAPI getUser].employeenum.length==18){
        shenfenzheng = [NSString stringWithFormat:@"%@********%@",[[TRUUserAPI getUser].employeenum substringToIndex:6],[[TRUUserAPI getUser].employeenum substringFromIndex:11]];
    }else{
        shenfenzheng = [TRUUserAPI getUser].employeenum;
    }
    userName.detailsStr = shenfenzheng;
    userName.type = 0;
    TRUPersonalDetailsModel *phone = [[TRUPersonalDetailsModel alloc] init];
    phone.titleStr = @"手机号";
    NSString *phoneshow;
    if([TRUUserAPI getUser].phone.length==11){
        phoneshow = [NSString stringWithFormat:@"%@****%@",[[TRUUserAPI getUser].phone substringToIndex:3],[[TRUUserAPI getUser].phone substringFromIndex:7]];
    }else{
        phoneshow = [TRUUserAPI getUser].phone;
    }
    phone.detailsStr = phoneshow;
