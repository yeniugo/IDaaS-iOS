...
//    phone.type = 1;
    ...
    if ([TRUUserAPI haveSubUser]) {
        TRUPersonalDetailsModel *subUser = [[TRUPersonalDetailsModel alloc] init];
        subUser.titleStr = @"其他身份账号";
//        subUser.selectStr = @"subUser";
        subUser.detailsStr = [NSString stringWithFormat:@"%d",[TRUUserAPI subUserCount]];
        subUser.type = 3;
        self.dataSourceArray = @[@[name,department,email,userName],@[subUser],@[unbind]];
    }else{
        self.dataSourceArray = @[@[name,department,email,userName],@[unbind]];
    }
