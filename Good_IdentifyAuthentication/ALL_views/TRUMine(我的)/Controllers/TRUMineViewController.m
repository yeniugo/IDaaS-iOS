...
- (void)viewDidLoad {
    [super viewDidLoad];
    if (!_imgsArr) {
        _imgsArr = @[@"apploginicon",@"deviceicon",@"bioicon",@"feedbackicon",@"abouticon",@"relieve"];
    }
    if (!_titlesArr) {
        _titlesArr = @[@"APP登录验证",@"设备管理",@"生物信息",@"问题反馈",@"关于我们",@"解除绑定"];
    }
    [self customUI];
...
-(void)customUI{
    ...
    _myCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 290, SCREENW, SCREENW/4.f * 3 - 2) collectionViewLayout:flowlayout];
    ...
    //适配4s，5，5s小屏幕
    if (SCREENW == 320){
        _myCollectionView.frame = CGRectMake(0, 230, SCREENW, SCREENW/4.f * 3 - 2);
        ...
        _myCollectionView.frame = CGRectMake(0, 350, SCREENW, SCREENW/4.f * 3 - 2);
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.row) {
        ...
        case 2://生物信息
        {
            TRUBioinformationViewController *bioVC = [[TRUBioinformationViewController alloc] init];
            [self.navigationController pushViewController:bioVC animated:YES];
        }
            break;
        case 3://问题反馈
        {
            TRUFeedbackViewController *feedvc = [[TRUFeedbackViewController alloc] init];
