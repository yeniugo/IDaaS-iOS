...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([classStr isEqualToString:@"TRUAboutUsViewController"]) {
        return;
    }
    NSString *titleStr;
    if (indexPath.section==1) {
        titleStr = self.titleArray[indexPath.section-1][indexPath.row];
//        self.title = titleStr;
    }
...
//            [weakSelf syncUserInfo];
    ...
    if ([classStr isEqualToString:@"TRUBuildingViewController"]) {
        pushVC = [[NSClassFromString(@"TRUBuildingViewController") alloc] init];
        TRUBuildingViewController *vc = pushVC;
        vc.title = self.titleArray[indexPath.section-1][indexPath.row];
    }
