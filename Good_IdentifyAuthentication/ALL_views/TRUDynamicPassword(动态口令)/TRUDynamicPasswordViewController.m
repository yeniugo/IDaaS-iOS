...
-(void)customUI{
    ...
    if (IS_IPAD) {
        self.progessimage.frame = CGRectMake(SCREENW*0.22, 80, SCREENW*0.56, SCREENW/2.f+30);
    }
    ...
    if (IS_IPAD) {
        self.numberView.frame = CGRectMake(SCREENW/12.f, (SCREENW/2.f+30)*0.4 , (SCREENW/3.f *2)*0.6, 40.0 * PointHeightRatio6);
    }
//    self.numberView.text = @"000000";
    [self.numberView setNumberStr:@"000000" isFirst:!self.firstRun];
    ...
    if (IS_IPAD) {
        self.refreshBtn.y = self.refreshBtn.y+100;
        self.syncLabel.y = self.syncLabel.y + 100;
    }
