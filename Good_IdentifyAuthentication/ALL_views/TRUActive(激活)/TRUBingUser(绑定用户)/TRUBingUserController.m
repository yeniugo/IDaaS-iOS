...
- (void)viewDidLoad {
    ...
    if (activeStr.length>0) { // 固定ip activeStr.length>0
        ...
        if (arr.count>0) {
            ...
            if ([modeStr isEqualToString:@"1"]) {//激活方式 激活方式(1:邮箱,2:手机,3:工号)
                ...
                isEmployee = YES;
                _inputoneTF.placeholder = @"请输入您的工号/手机号";
                _numView.hidden = NO;
                _sendBtn.hidden = YES;
                _iphoneEmialView.hidden = YES;
            }
        }
        [_inputoneTF addTarget:self action:@selector(passwordvalueChanged:)  forControlEvents:UIControlEventAllEditingEvents];
        [_inputpasswordTF addTarget:self action:@selector(passwordvalueChanged:)  forControlEvents:UIControlEventAllEditingEvents];
        [_inputphonemailTF addTarget:self action:@selector(passwordvalueChanged:)  forControlEvents:UIControlEventAllEditingEvents];
    }else{
...
- (void)passwordvalueChanged:(UITextField *)field{
...
-(void)requestCodeForUserEmployeenum:(NSString *)user type:(NSString *)type{
    ...
    [TRUhttpManager sendCIMSRequestWithUrl:[baseUrl stringByAppendingString:@"/mapi/01/init/apply4active"] withParts:paramsDic onResult:^(int errorno, id responseBody) {
        ...
        if (0 == errorno) {
            ...
            [weakSelf showHudWithText:@"账号密码错误"];
            [weakSelf hideHudDelay:2.0];
        }else{
            NSString *err = [NSString stringWithFormat:@"其他错误（%d）",errorno];
