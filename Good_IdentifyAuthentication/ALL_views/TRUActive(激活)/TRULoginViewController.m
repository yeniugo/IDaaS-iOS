...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if (1) {//说明已经切换了 固定ip  URLstr.length>0
    ...
    if (spname.length >0) {
        ...
        self.titleLabel.text = [NSString stringWithFormat:@"IDA统一身份认证"];
    }
}
...
//            [xindunsdk initEnv:@"com.example.demo" url:currentCims];
            bool result = [xindunsdk initCIMSEnv:@"com.example.demo" serviceUrl:currentCims devfpUrl:currentCims];
...
//            [HAMLogOutputWindow printLog:[NSString stringWithFormat:@"加密结果= %@",para]];
