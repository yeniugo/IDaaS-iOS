            ...
            if ([op isEqualToString:@"login"]) {
                ...
                if(0){
                    ...
                    if (userNo){
                        ...
                        [weakSelf showHudWithText:@"无效二维码"];
                        [weakSelf hideHudDelay:2.0];
                        [weakSelf performSelector:@selector(restartScan)  withObject:nil afterDelay:2.1];
                    }
                }
                ...
                [weakSelf showConfrimCancelDialogViewWithTitle:@"" msg:@"无效二维码，请确认二维码来源！" confrimTitle:@"确认" cancelTitle:@"" confirmRight:YES confrimBolck:^{
                    [weakSelf restartScan];
...
- (void)popMultipleAccountsWithPushModel:(TRUPushAuthModel*)pushModel{
    ...
    UINavigationController *pushNav1 = self.navigationController;
    [vc setBackBlock:^(NSString *userId) {
//        __weak typeof(self) weakSelf = self;
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//            [weakSelf dismissViewControllerAnimated:YES completion:nil];
//        });
        [pushNav1 popViewControllerAnimated:NO];
        YCLog(@"%@",pushNav1);
...
//            [weakSelf dismissViewControllerAnimated:YES completion:nil];
            [pushNav1 popViewControllerAnimated:YES];
        }];
        [pushNav1 pushViewController:authVC animated:YES];
