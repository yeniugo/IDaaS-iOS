...
@interface TRUPushViewController ()
...
@property (weak, nonatomic) IBOutlet UILabel *titleAccLB;


@property (weak, nonatomic) IBOutlet UILabel *accountLB;
@property (weak, nonatomic) IBOutlet UILabel *IPAddressLB;
...
@property (nonatomic, strong)LOTAnimationView *canclelotview;

@property (weak, nonatomic) IBOutlet NSLayoutConstraint *backBtnY;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *titleY;

@property (weak, nonatomic) IBOutlet NSLayoutConstraint *cancleBtnY;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *imgY;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *labelY;


@end

@implementation TRUPushViewController
{
    CAGradientLayer *gradientLayer;
    UIColor *startColor;
    UIColor *endColor;
    YCDownSlider *myslider;
}
- (void)viewDidLoad {
    [super viewDidLoad];
    [self customUI];
...
- (void)commonInit{
    [self startCounter];
    self.IPAddressLB.text = self.pushModel.ip;
    self.addressLB.text = self.pushModel.location;
    self.timeLB.text = self.pushModel.dateTime;
    NSString *userName = self.pushModel.username;
    self.titleAccLB.text = [NSString stringWithFormat:@"您正在通过【%@】登录",self.pushModel.appname];
...
- (IBAction)cancleBtnClick:(UIButton *)sender {
    ...
    [TRUhttpManager sendCIMSRequestWithUrl:[baseUrl stringByAppendingString:@"/mapi/01/verify/checktoken"] withParts:paramsDic onResult:^(int errorno, id responseBody) {
        ...
        if (errorno == 0) {
            ...
            myslider.hidden = YES;
            weakSelf.canclelotview.hidden = NO;
            [weakSelf.canclelotview playWithCompletion:^(BOOL animationFinished) {
...
//    YCLog(@"----->%@",authtype);
    ...
    if ([authtype isEqualToString:@"1"] || [authtype isEqualToString:@"0"] || [authtype isEqualToString:@"10"]) {
        ...
        if (str.length > 0 && [str isEqualToString:@"yes"]) {//第三方认证
            ...
            if ([weakSelf.pushModel.authtype isEqualToString:@"7"]) {//声纹
            ...
            if ([weakSelf.pushModel.authtype isEqualToString:@"6"]) {//人脸
                ...
                if (dd >= 0) {
                    ...
                    [weakSelf performSelector:@selector(dismissVC:) withObject:@"1" afterDelay:0.4];
                    
                }else{
                    [weakSelf showHudWithText:@"您所使用的人脸认证服务已到期，即将更新，请耐心等待。"];
                    [weakSelf hideHudDelay:2.0];
                    [weakSelf performSelector:@selector(dismissVC:) withObject:@"1" afterDelay:2.5];
                }
            }
            if ([weakSelf.pushModel.authtype isEqualToString:@"7"]) {//声纹
                [weakSelf checkIFlyModel];
            }
        }
...
- (void)showTimeoutTip{
    __weak typeof(self) weakSelf = self;
    [self showConfrimCancelDialogAlertViewWithTitle:@"请求超时" msg:@"由于您的认证请求已超时，国家医考中心无法确认您的身份，请重新发起认证请求" confrimTitle:@"OK" cancelTitle:nil confirmRight:NO confrimBolck:^{
