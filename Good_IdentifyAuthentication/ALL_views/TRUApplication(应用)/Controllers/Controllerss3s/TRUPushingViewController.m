...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *titileHeightConstraint;
...
//        [self setDismissBlock:^(BOOL confirm){
//            YCLog(@"checkIFlyModel confirm = %d",confirm);
//            if (confirm) {
...
//    UIViewController *rootVC1 = [appdelegate getRootWindowsViewController];
    //    UIViewController *rootVC1 = [[[UIApplication sharedApplication] delegate] window];
    //    if (rootVC != rootVC1) {
    //        YCLog(@"keywindow change");
//    }
...
- (void)showTimeoutTip{
    __weak typeof(self) weakSelf = self;
    [self showConfrimCancelDialogAlertViewWithTitle:@"请求超时" msg:@"由于您的认证请求已超时，请重新发起认证请求" confrimTitle:@"OK" cancelTitle:nil confirmRight:NO confrimBolck:^{
...
-(void)customUI{
    ...
    if(IS_IPAD){
        self.titleWidthConstraint.constant = SCREENW*0.8;
        self.bigTitleTopConstraint.constant = SCREENH/1024.0*110.0;
        self.titileHeightConstraint.constant = SCREENH/1024.0*180.0;
        self.logLBTopToBigTitleConstraint.constant = SCREENH/1024.0*180.0;
        self.confirmButtonTopConstraint.constant = SCREENH/1024.0*300.0;
        self.cancelBottomContraint.constant = SCREENH/1024.0*100.0;
    }
