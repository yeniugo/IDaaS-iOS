...
- (BOOL)application:(UIApplication *)app openURL:(NSURL *)url options:(NSDictionary<UIApplicationOpenURLOptionsKey,id> *)options{
    ...
    if ([str containsString:@"&"]) {
        ...
        if ([url.host isEqualToString:@"cims"]) {
            if (queryArr.count==3) {
                NSString *tokenStr = [[queryArr[1] componentsSeparatedByString:@"="] lastObject];
                self.token = tokenStr;
                self.urlstr = [[[queryArr lastObject] componentsSeparatedByString:@"="] lastObject];
                self.thirdAwakeTokenStatus = 10;
                if ([TRUFingerGesUtil getLoginAuthGesType] != TRULoginAuthGesTypeNone || [TRUFingerGesUtil getLoginAuthFingerType] != TRULoginAuthFingerTypeNone){
                    [self useTokenFromWeb];
                }else{
                    
                }
            }else if(queryArr.count==2){
                NSString *tokenStr = [[queryArr[1] componentsSeparatedByString:@"="] lastObject];
                self.token = tokenStr;
//                self.urlstr = [[[queryArr lastObject] componentsSeparatedByString:@"="] lastObject];
                if ([TRUFingerGesUtil getLoginAuthGesType] != TRULoginAuthGesTypeNone || [TRUFingerGesUtil getLoginAuthFingerType] != TRULoginAuthFingerTypeNone){
