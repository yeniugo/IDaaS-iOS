...
//    [self initMSC];
    ...
    DDLogError(@"app launch");
//    DDLogError(@"error test launch");
    //初始化Bugly
//    [self initBugly];
    //更新公司信息
//    [self requestSPinfo];
...
//    [self checkUpdataWithPlist];
    ...
    if(isCustom){
        ...
        imgUrlStr = [TRUCompanyAPI getCompany].start_up_img_url;
    }
...
//                    [HAMLogOutputWindow printLog:@"TRUBaseTabBarController1"];
                    TRUBaseNavigationController *baseNav = [[TRUBaseNavigationController alloc] initWithRootViewController:[[TRUBaseTabBarController alloc] init]];
...
- (void)changeRootVC{
    ...
    self.window.rootViewController = nil;
//    [HAMLogOutputWindow printLog:@"TRUBaseTabBarController2"];
...
- (void)changeRootVCWithInfo{
    //    [TRUTimeSyncUtil syncTimeWithResult:nil];
//    [HAMLogOutputWindow printLog:@"TRUBaseTabBarController3"];
    self.window.rootViewController = nil;
    TRUBaseTabBarController *tabvc = [[TRUBaseTabBarController alloc] init];
...
- (void)changeAvtiveRootVC{
    //只要跳转到激活页面，就把手势/指纹清空
//    [HAMLogOutputWindow printLog:@"更新控制器"];
    [TRUFingerGesUtil saveLoginAuthGesType:TRULoginAuthGesTypeNone];
    [TRUFingerGesUtil saveLoginAuthFingerType:TRULoginAuthFingerTypeNone];
    TRULoginViewController *loginVC = [[TRULoginViewController alloc] init];
//    TRUBaseNavigationController *nav = [[TRUBaseNavigationController alloc] initWithRootViewController:loginVC];
...
- (BOOL)application:(UIApplication *)app openURL:(NSURL *)url options:(NSDictionary<UIApplicationOpenURLOptionsKey,id> *)options{
    YCLog(@"openURL");
    NSString *log = [NSString stringWithFormat:@"openurl = %@",url.absoluteString];
    DDLogError(log);
    ...
    if ([host isEqualToString:@"auth"]) {
        ...
        if (urlArray.count==2) {
            ...
            self.appid = appid;
//            [HAMLogOutputWindow printLog:[NSString stringWithFormat:@"1appid = %@",appid]];
        ...
        if (urlArray.count==3) {
            NSString *appid = [[urlArray[0] componentsSeparatedByString:@"="] lastObject];
            self.appid = appid;
//            [HAMLogOutputWindow printLog:[NSString stringWithFormat:@"2appid = %@",appid]];
            NSString *type = [[urlArray[1] componentsSeparatedByString:@"="] lastObject];
...
//            urlStr = [NSString stringWithFormat:@"%@?token=%@",weakSelf.soureSchme,tokenDic[@"token"]];
        ...
        //                [HAMLogOutputWindow printLog:@"1111"];
//        [HAMLogOutputWindow printLog:[NSString stringWithFormat:@"返回scheme=%@",urlStr]];
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil message:urlStr preferredStyle:UIAlertControllerStyleAlert];
...
//
//                [HAMLogOutputWindow printLog:@"configRootBaseVCForApplication1"];
                if ([self.window.rootViewController isKindOfClass:[TRUAdViewController class]]) {
...
//                    [HAMLogOutputWindow printLog:@"001"];
                    UINavigationController *rootnav = self.window.rootViewController;
//                    if (![[rootnav.viewControllers firstObject] isKindOfClass:[TRUBaseTabBarController class]]) {
...
//        [HAMLogOutputWindow printLog:@"configRootBaseVCForApplication2"];
        if ([self.window.rootViewController isKindOfClass:[TRUAdViewController class]]) {
            [self openApplication:[UIApplication sharedApplication] WithOptions:self.launchOptions];
        }
        if(type==2){
...
//            [HAMLogOutputWindow printLog:@"TRUBaseTabBarController4"];
...
//        self.isNeedPush = NO;
    ...
    [self showlock];
//    [HAMLogOutputWindow printLog:@"进入后台1"];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//        [HAMLogOutputWindow printLog:@"进入后台2"];
        if (application.applicationState != UIApplicationStateActive) {
//            [HAMLogOutputWindow printLog:@"进入后台3"];
...
- (void)showlock{
    ...
    UIViewController *vc = [self getCurrentVC]; //截取当前视图为图片
...
//    }
    [self beginBackTask];
    DDLogError(@"applicationDidEnterBackground");
...
//    [HAMLogOutputWindow printLog:@"test20"];
    [TRULockSWindow dismissAuthViewAndCleanStatus];
    self.pushCompletionBlock = nil;
    self.appCompletionBlock = nil;
    
    YCLog(@"applicationWillResignActive");
    [self endBackTask];
}
...
- (void)applicationWillEnterForeground:(UIApplication *)application {
    ...
    DDLogError(@"applicationWillEnterForeground");
...
//                    [HAMLogOutputWindow printLog:@"test2"];
                ...
                if ([TRUFingerGesUtil getLoginAuthGesType] != TRULoginAuthGesTypeNone || [TRUFingerGesUtil getLoginAuthFingerType] != TRULoginAuthFingerTypeNone) {
                    ...
                    if (!self.thirdAwakeTokenStatus) {
//                        [TRUEnterAPPAuthView showAuthView];
//                        [HAMLogOutputWindow printLog:@"test3"];
                        [TRULockSWindow showAuthView];
                    }else{
//                        [TRUEnterAPPAuthView dismissAuthView];
//                        [HAMLogOutputWindow printLog:@"test4"];
                        [TRULockSWindow dismissAuthView];
                    }
                }
            }
...
- (void)applicationWillTerminate:(UIApplication *)application {
    ...
    DDLogError(@"applicationWillTerminate");
    [[NSUserDefaults standardUserDefaults] synchronize];
}
#pragma mark - 版本更新
-(void)checkVersion{
...
-(void)checkNewAppUpdate:(NSString *)appInfo updateURL:(NSString *)updateURL withFouce:(BOOL)fouce{
    ...
    DDLogError(@"update,appinfo=%@,updateURL=%@,fouce=%d,version=%@",appInfo,updateURL,fouce,version);
    CGFloat dispatchTime;
...
- (void)checkUpdataWithPlist{
    ...
    [manager GET:updateUrl parameters:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            DDLogError(@"1,url=%@",updateUrl);
            NSString *url = responseObject[@"url"];
            if (url.length) {
                DDLogError(@"2,url=%@",updateUrl);
                if ([url hasPrefix:@"https://"]) {
                    DDLogError(@"3,url=%@",updateUrl);
                    if (![url containsString:@"待配置"]) {
                        DDLogError(@"4,url=%@",updateUrl);
                        [weakSelf getPlistWithURL:url];
                    }
                }else{
                    url = [NSString stringWithFormat:@"%@%@",baseUrl,url];
                    DDLogError(@"5,url=%@",updateUrl);
                    if (![url containsString:@"待配置"]) {
                        DDLogError(@"6,url=%@",updateUrl);
                        [weakSelf getPlistWithURL:url];
                    }
        ...
        DDLogError(@"success,url=%@",updateUrl);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        YCLog(@"error");
        DDLogError(@"error,url=%@",updateUrl);
    }];
}
...
- (void)getPlistWithURL:(NSString *)url{
    ...
    [manager GET:url parameters:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        YCLog(@"bundleidStr = %@",bundleidStr);
        DDLogError(@"success,url=%@",url);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        YCLog(@"error");
        DDLogError(@"error,url=%@",url);
    }];
}
