...
@property (nonatomic, strong) NSMutableArray             *selectedTempView;
//手势滑动经过的点的连线
...
- (void)p_initView
{
    ...
    for (int i = 0; i<9 ; i++) {
        YZXPointView *pointView = [[YZXPointView alloc] initWithFrame:CGRectMake((i % 3) * (YZX_VIEW_WIDTH / 2.0 - 31.0) + 1, (i / 3) * (YZX_VIEW_HEIGHT / 2.0 - 31.0) + 1, 60, 60)
                                                         withID:[NSString stringWithFormat:@"[%d]",i]];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    if (_gestureBlock) {
        ...
        if (self.selectedView.count < 4) {
            ...
            if (self.gestureErrorBlock) {
                self.gestureErrorBlock();
            }
            return;
        }
...
//        if (_unlockBlock) {
//            self.unlockBlock(YES,self.selectedView);
//        }
//
//    }else {//解锁失败
...
//        if (_unlockBlock) {
//            self.unlockBlock(NO,self.selectedView);
//        }
//    }
}
...
- (void)p_drawLines
{
    ...
    self.lineLayer.strokeColor = YZX_RGB_COLOR(206.0, 42.0, 43.0, 1.0).CGColor;
