...
-(void)setIsInfo:(BOOL)isInfo{
    _isInfo = isInfo;
    _borderLayer.fillColor = [UIColor lightGrayColor].CGColor;
    self.centerLayer.hidden = isInfo;
}
...
- (void)setIsSuccess:(BOOL)isSuccess
{
    ...
    if (_isSuccess) {
        if (_isInfo) {
            ...
            self.centerLayer.fillColor = [UIColor lightGrayColor].CGColor;
        }
...
- (void)setIsError:(BOOL)isError
{
    ...
    if (_isError) {
        if (_isInfo) {
            self.borderLayer.fillColor = YZX_RGB_COLOR(222.0, 64.0, 60.0, 1).CGColor;
        }else{
            self.borderLayer.fillColor = YZX_RGB_COLOR(222.0, 64.0, 60.0, 0.5).CGColor;
